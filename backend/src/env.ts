...
const EnvSchema = z.object({
  ...
  BOOTSTRAP_ADMIN_USERNAME: z.string().trim().min(3).max(32).regex(/^[a-zA-Z0-9._-]+$/).optional(),
  BOOTSTRAP_ADMIN_PASSWORD: z.string().min(12).optional(),
  BOOTSTRAP_ADMIN_PASSWORD_HASH: z.string().min(20).optional(),
  BOOTSTRAP_SEED_DATA: boolEnv(!isProd),
  MYSQL_ENABLED: boolEnv(isProd),
  MYSQL_HOST: z.string().default("127.0.0.1"),
  MYSQL_PORT: z.coerce.number().int().positive().default(3306),
  MYSQL_USER: z.string().default("root"),
  MYSQL_PASSWORD: z.string().default(""),
  MYSQL_DATABASE: z.string().default("car_platform"),
  MYSQL_AUTO_CREATE_DATABASE: boolEnv(!isProd),
  ...
  UPLOAD_IMAGE_MAX_BYTES: z.coerce.number().int().positive().default(8 * 1024 * 1024),
  CARD_TO_CARD_DESTINATION_CARD: z.string().trim().min(12).max(32).default("6037-9979-0000-1234"),
})
